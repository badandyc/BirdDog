...
if [[ "$BIRDDOG_MODE" == "full" ]]; then
    ...
    # Permanent deterministic radio naming baked into the golden image.
    # No runtime service needed — udev assigns names by driver at every
    # boot before networking starts.
    #
    ...
    cat > /etc/udev/rules.d/72-birddog-radios.rules << 'UDEV_EOF'
# BirdDog deterministic radio naming — assigned by driver at boot
...
UDEV_EOF
    ...
    # Block onboard WiFi at boot — reduces RF congestion
    cat > /etc/systemd/system/birddog-block-onboard-wifi.service << 'SVC_EOF'
[Unit]
Description=BirdDog — block onboard WiFi (wlan0)
...
[Service]
Type=oneshot
ExecStart=/bin/sh -c 'ip link set wlan0 down 2>/dev/null; rfkill block wifi 2>/dev/null; true'
RemainAfterExit=yes
