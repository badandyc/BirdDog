...
normalize_and_join() {
    ...
    iw dev wlan1 set type mp >> "\$LOG" 2>&1 || return 0
...
do
    ...
    if ! interface_exists; then
        [[ "\$STATE" != "WAIT_INTERFACE" ]] && log_state "WAIT_INTERFACE"
        STATE="WAIT_INTERFACE"
        sleep 2
        continue
    fi
    ...
    if [[ "\$STATE" == "CONVERGING" ]]; then
        [[ "\$PEER_FOUND" -eq 1 ]] && log_state "STEADY" && STATE="STEADY"

    elif [[ "\$STATE" == "STEADY" ]]; then
        (( DELTA > SUSPECT_THRESHOLD )) && log_state "SUSPECT" && STATE="SUSPECT"
