    ...
    for (let p = 0; p < totalPages; p++) {
        ...
        batch.forEach(name => {
            ...
            if (isLive) {
                ...
                // transparent interceptor catches double tap without blocking video
                const interceptor = document.createElement('div');
                interceptor.style.cssText = 'position:absolute;top:0;left:0;width:100%;height:100%;z-index:5;';
                tile.appendChild(interceptor);
            } else {
...
function attachTileEvents(tile) {
    // attach to tile itself — interceptor div catches taps over iframe
    const handler = (e) => {
    ...
    tile.addEventListener('touchend', handler);
